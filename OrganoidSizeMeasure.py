...
#	for subfolder in subfolders:
	...
	log.write("File Name, Feret, MinFeret, Area, Circularity, Number of Buds \n")
	...
	for filename in os.listdir(inputDirectory): 
		if filename.endswith(".jpg"):
			...
			if maxArea > 0:				
				log.write(filename + ',' + str(table.getValue("Feret", index)) + ',' + str(table.getValue("MinFeret", index)) + ',' + str(table.getValue("Area", index)) + ',' + str(table.getValue("Circ.", index)) + ',' + str(len(table.getColumn(0))-1))
			else:
