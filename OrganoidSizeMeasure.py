...
with open(outputDirectory + "output.csv", "w") as log:
	...
	log.write("Subfolder, File Name,Feret,MinFeret,Average Feret,Area,Equivalent Circle Diameter, Ellipse Major, Ellipse Minor, Circularity,Roundness,Solidity,Number of Buds\n")
	subfolders = []
	
	for subfolder in os.listdir(inputD):
		if os.path.isdir(inputD + subfolder):
	...
	for subfolder in subfolders:
 		...
 		inputDirectory = inputD + subfolder 
		...
		for filename in os.listdir(inputDirectory): 
			if filename.endswith(".TIF"):      # add file name thing here eg if 4x.TIF
				...
				if maxArea > 0:				
					...
					log.write(str(subfolder) + ',' + filename + ',' + str(table.getValue("Feret", index)) + ',' + str(table.getValue("MinFeret", index)) + ',' + str((table.getValue("MinFeret", index)+table.getValue("Feret", index))/2) + ','  + str(table.getValue("Area", index)) + ',' + str(diameter) + ',' + str(table.getValue("Major", index)) + ','+ str(table.getValue("Minor", index)) + ','+ str(table.getValue("Circ.", index)) + ',' +str(table.getValue("Round", index)) + ',' + str(table.getValue("Solidity", index)) + ',' + str(buds))
				else:
					log.write(str(subfolder) + ',' + filename + ",NA,NA,NA,NA,NA,NA,NA,NA,NA,NA,NA")
